...
const tipoDocumentoIdentidad = document.getElementById(
...
const forma_pago_particionado = document.getElementById(
    "forma_pago_particionado",
);
...
function sumaTotal() {
    ...
    for (let i = 0; i < cantidades.length; i++) {
        total +=
            parseFloat(cantidades[i].textContent) *
            parseFloat(precios[i].textContent);
    }
    ...
    fetch(urlgeneral + "/consultar_dni_ruc", {
        ...
        .then((data) => {
            if (data.exception == "existe_base_datos") {
                ...
                nameCustomer.value =
                    data.original.nombres +
                    " " +
                    data.original.apellidoPaterno +
                    " " +
                    data.original.apellidoMaterno;
            }
        });
}
...
function banco_particionado(id) {
    fetch(urlgeneral + "/bancos-venta")
        .then((res) => res.json())
        .then((data) => {
            const banco_particionado_ = document.getElementById(
                "banco_particionado" + id,
            );
            let html = `<option value="">Seleccionar...</option>`;
...
addParticionado.addEventListener("click", (e) => {
    ...
    const textForma =
        forma_pago_particionado.options[forma_pago_particionado.selectedIndex]
            .text;
    ...
    const number_operation = document.querySelector(
        "#number_operation" + forma,
    );
    ...
    fetch(urlgeneral + "/caja/validar")
        ...
        .then((data) => {
            if (data.status == 1) {
                ...
                document.getElementById("mensaje_caja").textContent =
                    data.mensaje;
                document
                    .getElementById("ir_caja")
                    .setAttribute("href", urlgeneral + "/caja");
            }
        });
}
