    ...
    for (let indice = 0; indice < data.length; indice++) {
        ...
        if (data[indice].roles && data[indice].roles.length > 0) {
            data[indice].roles.forEach(function (role) {
                rolesHtml +=
                    '<label class="btn btn-success me-1">' + role + "</label>";
            });
        }
        ...
        contenidoHtml +=
            ...
            urlGeneral +
            "/perfil/" +
            (data[indice].img ? data[indice].img : "default.png") +
            "' alt='' class='img-thumbnail' width='50px'></td>";

        // Actions
        contenidoHtml += "<td>";
        contenidoHtml +=
            '<a class="btn btn-info me-1" href="' +
            urlGeneral +
            "/users/" +
            data[indice].id +
            '"><i class="bx bxs-show label-icon"></i> Ver</a>';
        ...
        if (typeof canEdit !== "undefined" && canEdit) {
            contenidoHtml +=
                ...
                "/users/" +
                data[indice].id +
                '/edit"><i class="bx bx-pencil label-icon"></i> Editar</a>';
        }
