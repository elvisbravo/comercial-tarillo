    ...
    $.get(urlgeeneral+"/subcategorias/listadocategorias",function(data){
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            if(valor==data[index].id){
                ...
                contenido += "<option value='" + data[index].id + "' selected>" + data[index].categoria + "</option >";

            }else{

                contenido += "<option value='" + data[index].id + "' >" + data[index].categoria + "</option >";
...
function llenarsubcategorias(data){
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if(data[i].estado==0){
        contenido += "<td style='padding:1px;text-align:center'> Inactivo</td>";
    ...
    contenido +=' <button type="button" onclick="abrimodal('+ data[i].id +')" class="btn btn-info waves-effect btn-label waves-light" data-bs-toggle="modal" data-bs-target="#staticBackdrop"><i class="fas fa-edit"></i></button>'
    if(data[i].estado==0){
 ...
 //GUARDAR LOS DATOS DE SECTOR
    $("#guardar").on("click",function(){
        ...
        var selectcategoria_id = document.getElementById("categoria_id"); /*Obtener el SELECT */
        var categoria_id = selectcategoria_id.options[selectcategoria_id.selectedIndex].value;
